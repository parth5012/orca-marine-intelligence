...
| Env | How | Config |
|-----|-----|--------|
| Prod (Vercel + Render) | Vercel Cron → `GET /api/cron/pfz` → `POST /api/pfz/refresh` | `infra/vercel.json` → `crons: [{path:/api/cron/pfz, schedule:"30 6 * * *"}`. **Required on Vercel:** `BACKEND_API_URL=https://orca-marine-intelligence-api.onrender.com` + `CRON_SECRET`. **Required on Render backend:** same `CRON_SECRET`. Note: Vercel reads `vercel.json` at project root — copy `infra/vercel.json` contents if dashboard uses another path. |
