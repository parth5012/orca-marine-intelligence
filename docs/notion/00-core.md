...
| **M-D** | **Frontend & Maps** | `frontend/` + `diagrams/` | `app/page.tsx` (shell), `MapView`, `ChatPanel`, `SafetyBadge`, `LanguageSwitch`, `bhashini.ts`, `geo.ts`, `route.ts` | Has live sample at `https://cron-system.vercel.app/orca/map/` — convert `diagrams/map-prototype.html` → `MapView.tsx` |
